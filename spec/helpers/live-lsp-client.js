...
class LiveLspClient {
  ...
  async start() {
    ...
    this.initializeResult = await this.request("initialize", {
      ...
      clientInfo: {
        name: "Lumine adapter integration specs",
        version: "1.0.0",
      },
      rootUri,
    ...
    await this.waitFor(
      () =>
        this.configurationRequests.some(({ section }) => section === "graphql-config") &&
        this.configurationRequests.some(({ section }) => section === "vscode-graphql"),
      "GraphQL workspace configuration requests",
    );
...
// leaves the document unchanged and the spec waits out its timeout for a
// revalidation the server had no reason to publish. Fail at the edit, where the
// reason is still legible.
